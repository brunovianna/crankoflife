...
#define CELL_ROWS 67 //135 / 3
#define CELL_COLS 120 //240 / 3


// Array of cells
bool cells[CELL_COLS][CELL_ROWS] ; 
// Buffer to record the state of the cells and use this while changing the others in the interations
bool cells_buffer [CELL_COLS][CELL_ROWS] ; 

//trying to create a buffer for the image and draw it at once
uint8_t lcd_buffer [240][134][2] ; 
...
uint32_t rgb2color(int r, int g, int b) {
    ...
    for (int y=0; y<CELL_ROWS; y++) {
      ...
      for (int xx=x-1; xx<=x+1;xx++) {
        for (int yy=y-1; yy<=y+1;yy++) {  
          if (((xx>=0)&&(xx<CELL_COLS))&&((yy>=0)&&(yy<CELL_ROWS))) { // Make sure you are not out of bounds
...
//trying to create a buffer for the image and draw it at once
void fill_buffer_rect (int start_x,int start_y,int w,int h, uint16_t c) {
    for (int x = 0;x<start_x+w+1;x++) {
        for (int y = 0;y<start_y+h+1;y++) {
            lcd_buffer[x][y][0] = c << 8;
            lcd_buffer[x][y][1] = c >> 8;
        }
    }
}
...
void app_main(void)
{
    ...
    //     printf("disable sleep ok");
    // } else {
    //     printf("disable sleep problem");
    // }

    struct timeval now, last_iteration;
    ...
    srand(adc1_get_raw(ADC1_CHANNEL_8));



    // How likely for a cell to be alive at start (in percentage)
    int probabilityOfAliveAtStart = RAND_MAX * 0.28; //28%

    printf("probablilty, %d\n", probabilityOfAliveAtStart);
    printf("random, %d\n", rand());
    ...
    uint32_t alive = rgb2color(0, 150, 64);
    //uint32_t dead = TFT_BLACK;

    // Initialization of cells
    for (int x=0; x<CELL_COLS; x++) {
        for (int y=0; y<CELL_ROWS; y++) {
        bool state = true;
        if (rand () > probabilityOfAliveAtStart) { 
            state = false;
        }
        else {
            state = true;
    ...
    while (true) {
        ...
        if (now.tv_usec > last_iteration.tv_usec + interval) {
            iteration();
            //fillScreen(TFT_BLACK);
            for (int x=0; x<CELL_COLS; x++) {
                for (int y=0; y<CELL_ROWS; y++) {
                    if (cells[x][y]) {
                        fillRect (x*cell_size,y*cell_size,cell_size,cell_size,alive);
                        //fill_buffer_rect (x*cell_size,y*cell_size,cell_size,cell_size,SWAPBYTES (alive));
                    } else {
                        fillRect (x*cell_size,y*cell_size,cell_size,cell_size,0x00);
                    }
                }
                fillRect (x*cell_size,134,cell_size,1,0x00);
            }
            // setAddress( 0, 0,  239, 133);
            // lcd_data(&lcd_buffer[0][0][0],sizeof(lcd_buffer));
    ...
    for (int i=0;i<_width;i++)
        drawPixel(20,i,rgb2color(255,0,255));
    ...
    for (int i=0;i<_width;i++)
        drawPixel(10,i,rgb2color(0,255,0));
